...
Original file is located at
    https://colab.research.google.com/drive/1FUYU8id3SWfmHALj--u_kiI7Ek9WSLa4
"""
...
movies_df['year'] = movies_df.title.str.extract('(\(\d\d\d\d\))',expand=False) #we're storing it in this variable
...
#Every genre is separated by a | so we simply have to call the Python split function on |
...
#So let's clean this up a bit by resetting the index and dropping the movieId, title, genres and year columns.
...
#^User profile: weights for every of the user's preferences
#Lets test it out extracting the genre table from the original dataframe

genreTable = moviesWithGenres_df.set_index(moviesWithGenres_df['movieId'])
#And drop the unnecessary information
genreTable = genreTable.drop('movieId', 1).drop('title', 1).drop('genres', 1).drop('year', 1)
genreTable.head()
genreTable.shape #give you size

#You get the table first, then drop the uncessary information
